...
async function runMigrations({ closePool = false } = {}) {
  ...
  try {
    ...
    // Table: promotions (publicités et promotions)
    await client.query(`
      CREATE TABLE IF NOT EXISTS promotions (
        ...
        frequence VARCHAR(50) DEFAULT 'chaque_ouverture' CHECK (frequence IN ('chaque_ouverture', 'une_fois_par_jour', 'une_seule_fois')),
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
      )
    `);
    ...
    await client.query('CREATE INDEX IF NOT EXISTS idx_promotions_actif ON promotions(actif)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_promotions_dates ON promotions(date_debut, date_fin)');
